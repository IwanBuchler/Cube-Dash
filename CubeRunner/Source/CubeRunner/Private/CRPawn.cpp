...
void ACRPawn::Tick(float DeltaTime)
{
    ...
    if(!bGameEnded)
    {
        FVector Impulse = FVector(Mass* FowardForce *DeltaTime, 0.0f, 0.0f);
...
void ACRPawn::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
    ...
    InputComponent->BindAction("Jump", IE_Pressed, this, &ACRPawn::JumpUp);
...
void ACRPawn::JumpUp()
{
    FVector Impulse1 = FVector(0.0f, 0.0f, 500.0f);
    Cube->AddImpulse(Impulse1,"",true);
}
